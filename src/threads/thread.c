...
{
	if(unblocked_tick > ticks)
	{
		unblocked_tick = ticks;
	}
	return;
}
...
void thread_sleep(int64_t ticks){
	...
	set_unblocked_tick(current_thread->restart_tick = ticks);
...
void timer_wakeup(int64_t restart_tick){
        ...
        restart_tick = INT64_MAX;

	struct list_elem *cur = list_begin(&blocked_list);

        while(cur != list_end(&blocked_list)){
                
                current_thread = list_entry(list_front(&blocked_list),struct thread, elem);

                if(restart_tick>=current_thread->restart_tick)
		{
                  cur = list_pop_front(&blocked_list);
                  thread_unblock(current_thread);
                }
                else
			cur = list_next(cur);
                        set_unblocked_tick(current_thread->restart_tick);
        }
}  
